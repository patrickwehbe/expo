...
test('initial state (firstRoute, no initialRouteName) focuses the first route at index 0', () => {
...
test('initial state (firstRoute, initialRouteName = baz) anchors first, focuses baz at index 1', () => {
...
test('initial state (firstRoute, initialRouteName = qux) puts qux right after the anchor', () => {
...
test('initial state (initialRoute) anchors on initialRouteName', () => {
...
test('initial state (order / none) keeps declaration order, index = focused declaration index', () => {
...
test('initial state (history) moves a non-first initial route to index 0', () => {
...
test('initial state (history) keeps declaration order when initial route is already first', () => {
...
test('rehydrates with backBehavior history preserving the persisted order, index by focused name', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('rehydrate (history) preserves persisted routes order verbatim', () => {
...
test('route names change (history) keeps surviving order and appends new tabs', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('route names change (history) preserves the focused route, index by name', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('route names change appends key-changed tabs at the end', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('route names change (firstRoute) re-arranges the back stack around the anchor', () => {
...
test('route names change falls back to the first rebuilt tab when the focused route is removed', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('navigate (order) keeps routes in declaration order and focuses the target', () => {
...
test('jumpTo (order) keeps declaration order; no-op when already focused', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar'],
    routeParamList: {},
...
test('navigate (firstRoute) arranges [first, focused, ...rest]', () => {
  const router = TabRouter({ backBehavior: 'firstRoute' });
  const options: RouterConfigOptions = {
    routeNames: ['a', 'b', 'c'],
    routeParamList: {},
...
test('navigate (initialRoute) arranges [initial, focused, ...rest]', () => {
  const router = TabRouter({ backBehavior: 'initialRoute', initialRouteName: 'b' });
  const options: RouterConfigOptions = {
    routeNames: ['a', 'b', 'c'],
    routeParamList: {},
...
test('ensures a fresh key for navigate when the route id changes (order, in place)', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
...
test('adds path on navigate if provided (order, in place)', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar', 'qux'],
    routeParamList: {},
...
test("doesn't remove existing path on navigate if not provided (order)", () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar', 'qux'],
    routeParamList: {},
...
test("doesn't merge params on navigate to an existing screen (order)", () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar', 'qux'],
    routeParamList: {
...
test('merges params on navigate to an existing screen if merge: true (order)', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar', 'qux'],
    routeParamList: {},
...
test('navigate (history) splices the target into the back-stack (worked sequence)', () => {
  const router = TabRouter({ backBehavior: 'history' });
  const options: RouterConfigOptions = {
...
test('back (history) walks the visit order (A->B->C->A->D)', () => {
  const router = TabRouter({ backBehavior: 'history' });
  const options: RouterConfigOptions = {
    routeNames: ['A', 'B', 'C', 'D', 'E'],
    routeParamList: {},
...
test('back (history) then navigate (A->B->C)', () => {
  const router = TabRouter({ backBehavior: 'history' });
  const options: RouterConfigOptions = {
    routeNames: ['A', 'B', 'C'],
    routeParamList: {},
...
test('back (history) returns null when at index 0', () => {
  const router = TabRouter({ backBehavior: 'history' });
  const options: RouterConfigOptions = {
    routeNames: ['bar', 'baz', 'qux'],
    routeParamList: {},
...
test('back (order) walks to the declaration-previous tab', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['a', 'b', 'c'],
    routeParamList: {},
...
test('back (firstRoute) goes to the first route, null when already first', () => {
  const router = TabRouter({ backBehavior: 'firstRoute' });
  const options: RouterConfigOptions = {
    routeNames: ['a', 'b', 'c'],
    routeParamList: {},
...
test('back (initialRoute) goes to the initial route, null when already initial', () => {
  const router = TabRouter({ backBehavior: 'initialRoute', initialRouteName: 'b' });
  const options: RouterConfigOptions = {
    routeNames: ['a', 'b', 'c'],
    routeParamList: {},
...
test('back (none) always returns null', () => {
...
test('replace (firstRoute) drops the replaced route so back is blocked', () => {
  const router = TabRouter({ backBehavior: 'firstRoute' });
  const options: RouterConfigOptions = {
    routeNames: ['one', 'two'],
    routeParamList: {},
...
test('replace (history) drops the replaced route so back skips it', () => {
  const router = TabRouter({ backBehavior: 'history' });
  const options: RouterConfigOptions = {
...
test('getStateForRouteFocus (firstRoute) arranges [anchor, focused, ...]', () => {
  const router = TabRouter({ backBehavior: 'firstRoute' });
...
test('getStateForRouteFocus (history) splices into the back-stack', () => {
  const router = TabRouter({ backBehavior: 'history' });
...
test('navigate to a preloaded route focuses it and drops it from preloadedRouteKeys (order keeps layout)', () => {
  const router = TabRouter({ backBehavior: 'order' });
  const options: RouterConfigOptions = {
    routeNames: ['baz', 'bar', 'qux'],
    routeParamList: {},
